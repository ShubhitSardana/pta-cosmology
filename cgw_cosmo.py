...
with open(os.path.join(config_dir, 'binary_params_Om.json'), 'r') as f:
    config = json.load(f)
...
# Build binary parameters dynamically from config
binary_params = {}
...
B1_mc = binary_params['B1']['mc']# * (1 + B1_z)  # Redshift Mass
B1_sig = binary_params['B1']['sig']
B1_lum_dist = binary_params['B1']['lum_dist']
B1_fgw = binary_params['B1']['fgw']# / (1 + B1_z)  # Redshift Frequency
...
B2_mc = binary_params['B2']['mc']# * (1 + B2_z)  # Redshift Mass
B2_sig = binary_params['B2']['sig']
B2_lum_dist = binary_params['B2']['lum_dist']
B2_fgw = binary_params['B2']['fgw']# / (1 + B2_z)  # Redshift Frequency
...
B4_ra, B4_dec = binary_params['B4']['ra'], binary_params['B4']['dec']
B4_gwtheta, B4_gwphi = binary_params['B4']['gwtheta'], binary_params['B4']['gwphi']
B4_z = binary_params['B4']['z']
B4_mc = binary_params['B4']['mc'] 
B4_sig = binary_params['B4']['sig']
B4_lum_dist = binary_params['B4']['lum_dist']
B4_fgw = binary_params['B4']['fgw'] 
B4_phase0, B4_psi = binary_params['B4']['phase0'], binary_params['B4']['psi']
B4_inc = binary_params['B4']['inc']

B5_ra, B5_dec = binary_params['B5']['ra'], binary_params['B5']['dec']
B5_gwtheta, B5_gwphi = binary_params['B5']['gwtheta'], binary_params['B5']['gwphi']
B5_z = binary_params['B5']['z']
B5_mc = binary_params['B5']['mc'] 
B5_sig = binary_params['B5']['sig']
B5_lum_dist = binary_params['B5']['lum_dist']
B5_fgw = binary_params['B5']['fgw'] 
B5_phase0, B5_psi = binary_params['B5']['phase0'], binary_params['B5']['psi']
B5_inc = binary_params['B5']['inc']
...
for bname, bp in binary_params.items():
    ...
    mcmc_start_dict[f"{bname}_log10_Mc"] = np.log10(bp['mc'])# * (1 + bp['z'])
    ...
    mcmc_start_dict[f"{bname}_log10_fgw"] = np.log10(bp['fgw']) #  / (1 + bp['z'])
...
if run_mcmc_analysis:
    ...
    with ProcessPoolExecutor(max_workers=min(num_psrs, os.cpu_count()//2)) as ex:
        ...
        for psr in enterprise_psrs:
            ...
            if cw:
                ...
                binaries = [
                    ...
                    dict(cos_gwtheta=np.cos(B4_gwtheta), gwphi=B4_gwphi, cos_inc=np.cos(B4_inc),
                        log10_mc=np.log10(B4_mc), log10_fgw=np.log10(B4_fgw),
                        log10_dist=np.log10(B4_lum_dist), phase0=B4_phase0, psi=B4_psi),
                    dict(cos_gwtheta=np.cos(B5_gwtheta), gwphi=B5_gwphi, cos_inc=np.cos(B5_inc),
                        log10_mc=np.log10(B5_mc), log10_fgw=np.log10(B5_fgw),
                        log10_dist=np.log10(B5_lum_dist), phase0=B5_phase0, psi=B5_psi),
                ]
...
# ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── #
    ...
    Om_glob = parameter.Uniform(0.1, 0.5)("Omega_m")
    ...
    # P2 Globals
    nameB4 = "B4"
    B4_log10_Mc_prior = parameter.Normal(mu=np.log10(B4_mc), sigma=B4_sig)("{}_log10_Mc".format(nameB4))
    B4_phase0_prior = parameter.Uniform(0.0, 2 * np.pi)("{}_phase0".format(nameB4))
    B4_costh = parameter.Constant(np.cos(B4_gwtheta))("{}_costheta".format(nameB4))
    B4_cosinc_prior = parameter.Uniform(-1.0, 1.0)("{}_cosinc".format(nameB4))
    B4_psi_prior = parameter.Uniform(0, np.pi)("{}_psi".format(nameB4))
    B4_phi = parameter.Constant(B4_gwphi)("{}_phi".format(nameB4))

    # P3 Globals
    nameB5 = "B5"
    B5_log10_Mc_prior = parameter.Normal(mu=np.log10(B5_mc), sigma=B5_sig)("{}_log10_Mc".format(nameB5))
    B5_phase0_prior = parameter.Uniform(0.0, 2 * np.pi)("{}_phase0".format(nameB5))
    B5_costh = parameter.Constant(np.cos(B5_gwtheta))("{}_costheta".format(nameB5))
    B5_cosinc_prior = parameter.Uniform(-1.0, 1.0)("{}_cosinc".format(nameB5))
    B5_psi_prior = parameter.Uniform(0, np.pi)("{}_psi".format(nameB5))
    B5_phi = parameter.Constant(B5_gwphi)("{}_phi".format(nameB5))

    tref_mod = 53000*86400.0

    curnm = common_red_noise_block(psd="powerlaw", prior="log-uniform",

    logmin=-18.0, logmax=-10.0, gammamin=1,
    ...
    for psr in enterprise_psrs:
        ...
        CW_B1 = cw_block_H0(
                        ...
                        log10_fgw=np.log10(B1_fgw), z=B1_z, psrTerm=True, evolve=evolve_val, phase_approx=phase_approx_val, 
                        tref=tref_mod, check=evolve_val, p_dist=p_dist_prior, Om0=Om_glob, #p_phase=p_phase_prior,
                        )
        CW_B2 = cw_block_H0(
                        ...
                        log10_fgw=np.log10(B2_fgw), z=B2_z, psrTerm=True, evolve=evolve_val, phase_approx=phase_approx_val, 
                        tref=tref_mod, check=evolve_val, p_dist=p_dist_prior, Om0=Om_glob, #p_phase=p_phase_prior,
                        )
        CW_B3 = cw_block_H0(
                        ...
                        log10_fgw=np.log10(B3_fgw), z=B3_z, psrTerm=True, evolve=evolve_val, phase_approx=phase_approx_val, 
                        tref=tref_mod, check=evolve_val, p_dist=p_dist_prior, Om0=Om_glob, #p_phase=p_phase_prior,
                        )
        CW_B4 = cw_block_H0(
                        log10_H0=log10_H0_glob, pdist=pdist_dict.get(psr.name), name=f'B4', costh=B4_costh, phi=B4_phi,
                        log10_Mc = B4_log10_Mc_prior, cosinc=B4_cosinc_prior, phase0=B4_phase0_prior, psi=B4_psi_prior,
                        log10_fgw=np.log10(B4_fgw), z=B4_z, psrTerm=True, evolve=evolve_val, phase_approx=phase_approx_val, 
                        tref=tref_mod, check=evolve_val, p_dist=p_dist_prior, Om0=Om_glob, #p_phase=p_phase_prior,
                        )
        CW_B5 = cw_block_H0(
                        log10_H0=log10_H0_glob, pdist=pdist_dict.get(psr.name), name=f'B5', costh=B5_costh, phi=B5_phi,
                        log10_Mc = B5_log10_Mc_prior, cosinc=B5_cosinc_prior, phase0=B5_phase0_prior, psi=B5_psi_prior,
                        log10_fgw=np.log10(B5_fgw), z=B5_z, psrTerm=True, evolve=evolve_val, phase_approx=phase_approx_val, 
                        tref=tref_mod, check=evolve_val, p_dist=p_dist_prior, Om0=Om_glob, #p_phase=p_phase_prior,
                        )
        ...
        if curn: model += curnm
        if cw: 
            if num_binaries==1: model += CW_B1
            if num_binaries==2: model += CW_B1 + CW_B2 
            if num_binaries==3: model += CW_B1 + CW_B2 + CW_B3
            if num_binaries==4: model += CW_B1 + CW_B2 + CW_B3 + CW_B4
            if num_binaries==5: model += CW_B1 + CW_B2 + CW_B3 + CW_B4 + CW_B5
...
# ──────────────────────────────────────── AITOFF PLOT ──────────────────────────────────────── #
    if add_inj:
        ...
        for psr in enterprise_psrs:
            ra_list.append(psr._raj)
            dec_list.append(psr._decj)
        ...
        pulsar_positions = [np.array([-np.cos(d)*np.cos(r), -np.cos(d)*np.sin(r), -np.sin(d)]) 
        ...
        sky_sensitivity = np.zeros((n_dec, n_ra))
        for i in range(n_dec):
            for j in range(n_ra):
                total = 0
                for pos in pulsar_positions:
                    fplus, fcross, _ = utils.create_gw_antenna_pattern(pos, gwtheta_grid[i,j], gwphi_grid[i,j])
                    total += fplus**2 + fcross**2
                sky_sensitivity[i,j] = total

        ra_dec_list = [] 
        for i in range (1, num_binaries+1): 
            ra = globals()[f"B{i}_ra"] 
            dec = globals()[f"B{i}_dec"] 
            ra_dec_list.append((ra, dec))

        coords_bin = SkyCoord([ra for ra, dec in ra_dec_list],
                            [dec for ra, dec in ra_dec_list],
        ...
        cbar = fig.colorbar(im, orientation='horizontal', pad=0.08)
        cbar.set_label('Log10(Relative Sensitivity)', fontsize=12)

        ax.scatter(ra1, dec1, s=30, color='green', marker='o', label='Pulsars')
        for i, name in enumerate(name_list):
            # We add a tiny offset so the text doesn't sit exactly on top of the dot
            ax.text(ra1[i] + 0.02, dec1[i] + 0.02, name, 
                    fontsize=7,        # Smaller font to avoid clutter
                    alpha=0.8,         # Slightly transparent
                    color='darkgreen', 
                    ha='left', 
                    va='bottom')
        ax.scatter(ra2, dec2, s=70, color='black', marker='*', label='Injected Binaries')
        for i, (r, d) in enumerate(zip(ra2, dec2), start=1):
            ax.text(r, d, chr(64+i), fontsize=12, ha='left', va='bottom', color='black')

        ax.legend(loc='upper right')
        ax.set_title('PTA Sky Sensitivity Map with Pulsars and Injected Binaries', fontsize=16, pad=20)
        plt.tight_layout()
...
# ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── #
    ...
    m = np.sqrt(40/len(parfiles)) 
    cov = np.eye(ndim) * 0.001**2
...
if make_corner_plot:
    ...
    if cw:
        for i in range (1, num_binaries+1):
            ...
            if i == 3: params_to_plot_names.append(r"P1 $\ \log_{10}M_c$")
            if i == 4: params_to_plot_names.append(r"P2 $\ \log_{10}M_c$")
            if i == 5: params_to_plot_names.append(r"P3 $\ \log_{10}M_c$")

        params_to_plot.append("log10_H0")
        params_to_plot.append("Omega_m")
        params_to_plot_names.append(r"$H_0$")
        params_to_plot_names.append(r"$O_M$")
